...
namespace Client
{
    public partial class frmDrawing : MetroForm
    {
        public frmDrawing()
        {
            InitializeComponent();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ds = Program.SQL.SelectAll("SELECT id FROM lists_drawings WHERE LOWER(name)=@name AND LOWER(revision)=@revision AND date=@date;");
                ...
                int i = Program.SQL.Insert("INSERT INTO lists_drawings (name,revision,date,entered,modified) VALUES (@name,@revision,@date,@ent,@ent)");
                if (i < 1)
                {
